...
for v0 in [0, 1, 0.858242]:
    sol = solve_ivp(vdp, [0, 1], [2, v0], t_eval=np.linspace(0, 1, 1000))
    ax.plot(sol.t, sol.y[0], 'k-', linewidth=.5)

ax.plot([0, 1], [2, 1.8], 'ko', markersize=8, clip_on=False)
...
ax.set_xticks(np.arange(0, 1.02, 0.02), minor=True)
ax.set_yticks(np.arange(1.4, 2.3, 0.1))
ax.set_yticks(np.arange(1.4, 2.22, 0.02), minor=True)
